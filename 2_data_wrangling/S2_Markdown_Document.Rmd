...
community-built packages (also refered to as libraries) which continue to
...
Don't use `=`: it will work, but it will cause confusion later. 
...
Here we select the first five elements of the `speed` vector.
...
Note that a vector only requires one position arguement, while a 
...
**Quick exercise:** What's the `pivot_longer` command for the quarterly 
...
To pivot the data back into its original wide format, we can use `pivot_wider`. 
